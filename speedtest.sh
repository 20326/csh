...
#
# Short: https://git.io/csh-speedtest.sh
# File name: speedtest.sh
# Description: Install latest version speedtest-go
...
## Project Vars
# Name
PROJECT_NAME="speedtest.sh"
# Release link
# Version link
VERSION_URL="https://api.github.com/repos/librespeed/speedtest-go/releases/latest"
...
show_help() {
  ...
  echo '  -d            : set speedtest path (default: %s/go)'
  echo '  -h, --help    :  Show help'
  echo ''
  echo '> bash <(curl -fsSL git.io/csh-speedtest.sh)'
  exit 0
}
...
install_binary() {
    ...
    if [ -n "${LISTEN_PORT}" ]; then
        sed -i "s/listen_port=8989/listen_port=${LISTEN_PORT}/g" ${INSTALL_PATH}/settings.toml
    fi
}
...
install_service() {
    ...
    BOLD "> nohup ${INSTALL_PATH}/speedtest-backend -c ${INSTALL_PATH}/settings.toml > ${INSTALL_PATH}/debug.log &"
}
